...
echo " ██║ ╚████║╚██████╔╝██████╔╝███████╗██╔╝ ██╗    ╚██████╗██║  ██║██║     ██║   ██║   ██║  ██║███████╗";
echo " ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝     ╚═════╝╚═╝  ╚═╝╚═╝     ╚═╝   ╚═╝   ╚═╝  ╚═╝╚══════╝";
echo ">>> Cosmovisor Automatic Installer for Zetachain | Chain ID : athens_7001-1 <<<";
echo -e "\e[0m"
...
REPO=https://zetachain-external-files.s3.amazonaws.com/binaries/athens3/latest/zetacored-ubuntu-22-amd64
GENESIS=https://snap.nodexcapital.com/zetachain/genesis.json
ADDRBOOK=https://snap.nodexcapital.com/zetachain/addrbook.json
PORT=226

# Set Vars
if [ ! $NODENAME ]; then
        read -p "hello@nodexemperor:~# [ENTER YOUR NODENAME] > " NODENAME
...
# Get testnet version of Zetachain
mkdir -p $HOME/$FOLDER/$COSMOVISOR/genesis/bin
wget -O $HOME/$FOLDER/$COSMOVISOR/genesis/bin/$BINARY $REPO
chmod +x $HOME/$FOLDER/$COSMOVISOR/genesis/bin/$BINARY
...
curl -L https://snap.nodexcapital.com/zetachain/zetachain-latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/$FOLDER
