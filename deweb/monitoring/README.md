...
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for deweb is `16657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for deweb is `16090` |
...
```
# enable prometheus
sed -i -e "s/prometheus = false/prometheus = true/" $HOME/.dewebd/config/config.toml
```
