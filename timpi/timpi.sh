...
echo " ██║ ╚████║╚██████╔╝██████╔╝███████╗██╔╝ ██╗    ╚██████╗██║  ██║██║     ██║   ██║   ██║  ██║███████╗";
echo " ╚═╝  ╚═══╝ ╚═════╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝     ╚═════╝╚═╝  ╚═╝╚═╝     ╚═╝   ╚═╝   ╚═╝  ╚═╝╚══════╝";
echo ">>> Cosmovisor Automatic Installer for Timpi Chain | Chain ID : TimpiChainTN2 <<<";
echo -e "\e[0m"

sleep 1

# Variable
SOURCE=Timpi-ChainTN
WALLET=wallet
BINARY=timpid
CHAIN=TimpiChainTN2
FOLDER=.TimpiChain
...
REPO=https://github.com/Timpi-official/Timpi-ChainTN.git
GENESIS=https://snap.nodexcapital.com/timpi/genesis.json
ADDRBOOK=https://snap.nodexcapital.com/timpi/addrbook.json
PORT=225
...
# Get testnet version of Timpi
cd $HOME
rm -rf $SOURCE
git clone $REPO
cd $SOURCE/cmd/TimpiChain
go build 
sudo mv TimpiChain timpid
...
PEERS="$(curl -sS https://rpc-t.timpi.nodestake.top/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
SEEDS=""
...
curl -L https://snap.nodexcapital.com/timpi/timpi-latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/$FOLDER
