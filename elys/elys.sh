...
GENESIS=https://snap.nodexcapital.com/elys/genesis.json
ADDRBOOK=https://snap.nodexcapital.com/elys/addrbook.json
PORT=219
...
#Set Peers & Seeds
PEERS="$(curl -sS https://elys-testnet-rpc.polkachu.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
...
curl -L https://snap.nodexcapital.com/elys/elys-latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/$FOLDER
