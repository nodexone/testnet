...
# Set up monitoring and alerting for nibiru validator
...
| KEY |VALUE |
|---------------|-------------|
| **bond_denom** | Denominated token name, for example, `unibi` for nibiru testnet. You can find it in genesis file |
| **bench_prefix** | Prefix for chain addresses, for example, `nibiru` for nibiru testnet. You can find it in public addresses like this **nibiru**_valoper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete_ |
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for nibiru is `16657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for nibiru is `16090` |
...
To add validator use command with specified `VALIDATOR_IP`, `NIBIRU_VALOPER_ADDRESS`, `nibiru_WALLET_ADDRESS` and `PROJECT_NAME`
```
$HOME/cosmos_node_monitoring/add_validator.sh VALIDATOR_IP NIBIRU_VALOPER_ADDRESS NIBIRU_WALLET_ADDRESS PROJECT_NAME
```

> example: ```$HOME/cosmos_node_monitoring/add_validator.sh 1.2.3.4 nibivaloper1zyyz4m9ytdf60fn9yaafx7uy7h463n7alv2ete nibi1zyyz4m9ytdf60fn9yaafx7uy7h463n7a05eshc nibiru```
