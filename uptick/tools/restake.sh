...
for (( ;; )); do
	...
	echo uptickd tx distribution withdraw-rewards $UPTICK_VALOPER_ADDRESS --from=$WALLET --commission --chain-id=$UPTICK_CHAIN_ID --yes
	...
	echo uptickd tx staking delegate $UPTICK_VALOPER_ADDRESS $AMOUNT_STRING --from $WALLET --chain-id $UPTICK_CHAIN_ID --yes
