...
function animate {
  ...
  if [ $? -ne 0 ]; then
    printf "\r[\033[0;31m✗\033[0m] $1 failed!\n"
    exit 1
  fi
...
sudo rm -rf /usr/local/go >/dev/null 2>&1 &
 ...
 eval $(echo 'export PATH=$PATH:/usr/local/go/bin' | sudo tee /etc/profile.d/golang.sh) >/dev/null 2>&1 &
 eval $(echo 'export PATH=$PATH:$HOME/go/bin' | tee -a $HOME/.profile) >/dev/null 2>&1 &
...
sudo ln -s $HOME/$FOLDER/$COSMOVISOR/current/bin/$BINARY /usr/local/bin/$ >/dev/null 2>&1 &
