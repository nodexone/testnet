...
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for sei is `16657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for sei is `16090` |
...
```
# enable prometheus
sed -i -e "s/prometheus = false/prometheus = true/" $HOME/.seid/config/config.toml
```
...
To add validator use command with specified `VALIDATOR_IP`, `SEI_VALOPER_ADDRESS`, `SEI_WALLET_ADDRESS` and `PROJECT_NAME`
```
$HOME/cosmos_node_monitoring/add_validator.sh VALIDATOR_IP sei_VALOPER_ADDRESS sei_WALLET_ADDRESS PROJECT_NAME
```
