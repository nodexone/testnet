...
# Set up monitoring and alerting for teritori validator
...
| **rpc_port** | Your validator `rpc` port that is defined in `config.toml` file. Default value for teritori is `19657` |
| **grpc_port** | Your validator `grpc` port that is defined in `app.toml` file. Default value for teritori is `19090` |
...
To add validator use command with specified `VALIDATOR_IP`, `TERITORI_VALOPER_ADDRESS`, `TERITORI_WALLET_ADDRESS` and `PROJECT_NAME`
```
